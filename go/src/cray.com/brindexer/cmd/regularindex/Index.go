...
func parseCommands() (string, int, *string, bool) {
	...
	return args[0], *threads, dbBase, *full
}

func main() {
	target, threads, dbBase, full := parseCommands()
	start := time.Now()
	...
	if level > 0 {
		...
		indexer := indexing.NewDirectoryIndexer(&indexNode, threads)
	...
	if dbcount > 0 {
		fmt.Println("Committed db records: ", dbcount)
